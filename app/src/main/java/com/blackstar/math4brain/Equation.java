...
public class Equation {
	...
	private double num1, num2, num3, a,b,c,d;
	public int eqType, eqType2, diff, randCount, ans, numPrev=0, mAns=0;
	public long fastestTime=Long.MAX_VALUE, startTime=0;
	public String prevEquations = "", sign="", fastestEquation, easyEqns="";
	...
	public String getEquation (){
		...
		if (eqType == 5){
			if(c>0) equation = (int)a+"/"+(int)b+" + "+(int)c+"/"+(int)d;
			else equation = (int)a+"/"+(int)b+" - "+(int)Math.abs(c)+"/"+(int)d;
			if(multi){
				return "("+equation+") "+sign+" "+(int)num3+" = ";
			}
			return equation+" = ";
	...
	public void createNew(){
		...
		//Powers
			...
			//adjust difficulty
			if(difficulty > 3 && ans <= 6){
				ok=false;
			}
			else if(difficulty > 2 && ans <= 3){
				ok=false;
			}else if(difficulty > 1 && ans == 0){
				ok=false;
			}
		}
		//Percent
			...
			boolean search = true;
			while (search){
				int uBound = 100+(difficulty*10), lBound=0;
				num1 = lBound + (int) ( Math.random()*(uBound - lBound) );
				num1 = (int)(num1/10)*10;
				...
				if (d1 == d2){
					...
					if (ans%5==0 || ans%4==0){
						//eliminate easy questions
						if (!((difficulty>2)&&((num1==0)||(num1==100)||(num2==100)||(num2==50)))){
							search = false;
						}
		...
		//Fractions
			...
			boolean search = true;
			while (search) {
				a = (int) (Math.random() * (difficulty * 2.5)) + 1;
				b = (int) (Math.random() * (difficulty * 2.5)) + 1;
				c = (int) (Math.random() * (difficulty * 2.5)) + 1;
				d = (int) (Math.random() * (difficulty * 2.5)) + 1;
				if (choice == 1 && (((a/b)+(c/d))%1 == 0 && (a/b)%1!=0)) {
					ans = (int)((a/b)+(c/d));
					search = false;
				}
				else if (choice == 0 && (a/b)-(c/d) > 0 && (((a/b)-(c/d))%1 == 0 && (a/b)%1!=0)){
					ans = (int)((a/b)-(c/d));
					c = 0-c;
					search = false;
				}
		...
		//Avoid easy equations
		if(easyEqns.contains(getEquation())) ok=false;
	...
	public boolean isFastestTime (){
		long eqnTime = System.currentTimeMillis()-startTime;
		if(eqnTime < fastestTime){
			fastestTime = eqnTime;
			fastestEquation = getEquation();
			return true;
		}
		return false;
	}

	public void avoidEquations(String eqn){
		easyEqns = eqn;
	}
